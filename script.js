...
function navigasiKe(pageName) {
  ...
  halaman.forEach((item, index) => item.classList.replace("show", "hidde"));
...
// membuat tombol play menjadi hijau ketika pengguna memasukan username
elements.usernameInput.addEventListener("input", e => {
  if (e.target.value !== "") {
    elements.gameBtn.style.backgroundColor = "green";
  } else {
    elements.gameBtn.style.backgroundColor = "gray";
  }
})
...
function updateDarah() {
  document.getElementById("darah").textContent = `${"❤".repeat(
    darah
  )}`;
  if (darah <= 0) {
    endGame();
